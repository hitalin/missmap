...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		console.log(`Fetching federations from ${seedServer}...`);
		...
		for (let i = 0; i < maxFetches; i++) {
			...
			console.log(`Response status: ${res.status} (offset: ${offset})`);
			...
			if (!res.ok) {
				...
				// 最初のリクエストで失敗したらエラーを返す、それ以降は取得済みデータで続行
				if (i === 0) {
					return json(
						{
							error: 'FETCH_FAILED',
		...
		console.log(`Total instances fetched: ${allInstances.length}`);
